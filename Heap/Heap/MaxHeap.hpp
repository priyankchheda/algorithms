/**
 * Heap Data Strucure - Max Heap (C++ implementation)
 *
 ...
 * keys present at all of it’s children. The same property must be recursively
...
namespace Heap {
     ...
     */
    template <typename T>
    MaxHeap<T>::MaxHeap(int capacity) : capacity(capacity) {
     ...
     */
    template <typename T>
    MaxHeap<T>::MaxHeap(T* array, int arrayLen) {
        capacity = arrayLen;
        data = new T[capacity + 1];
        size = arrayLen;
    ...
    /**
     * ExtractMax returns the maximum value (i.e. root) in the heap and
     * removes it from the heap.
     * @return maximum value from heap
     * @throw MaxHeap::Exception::EmptyHeap when trying to access item from
     *      empty heap
     */
    template <typename T>
    ...
    /**
     * Max returns the maximum value (i.e. root) in the heap, without
     * removing it.
     * @return maximum value from heap
     * @throw MaxHeap::Exception::EmptyHeap when trying to access item from
     *      empty heap
